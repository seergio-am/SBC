...
static SemaphoreHandle_t xDistanceSemaphore;
...
void ultrasonic_task1(void *pvParameters){//sensor alante
    ...
    while (true)
    {
        ...
        if (res == ESP_OK || (strcmp(buf, "forward") == 0 || strcmp(buf, "front_right") == 0 || strcmp(buf, "front_left") == 0))
        {
            // Adquiere el semáforo antes de actualizar distansia
            xSemaphoreTake(xDistanceSemaphore, portMAX_DELAY);
            distanciaS1 = distance;
            
            char dist[10];
            sprintf(dist, "%0.2f cm", distanciaS1 * 100);
            printf("Distancia DELANTE: %0.0f cm\n", distanciaS1 * 100);

            //parar si hay obstáculo
            if((distanciaS1*100) < 4){
                stop();
            }
            if((distanciaS1*100) >= 4){ //continuar hacia alante
                forward();
            }


            if((strcmp(buf, "forward") != 0 && strcmp(buf, "front_right") != 0 && strcmp(buf, "front_left") != 0) || strcmp(buf, "stop") == 0)
            // Libera el semáforo después de que se muestre la distancia del sensor
            xSemaphoreGive(xDistanceSemaphore);
            }
        else
        {
            printf("Alante Error %d: %s\n", res, esp_err_to_name(res));
        }
...
void ultrasonic_task2(void *pvParameters){//sensor atrás
    ...
    while (true)
    {
        ...
        if (res == ESP_OK || (strcmp(buf, "back") == 0 || strcmp(buf, "back_right") == 0 || strcmp(buf, "back_left") == 0))
        {
            // Adquiere el semáforo antes de actualizar distansia
            xSemaphoreTake(xDistanceSemaphore, portMAX_DELAY);
            distanciaS2 = distance;
            
            char dist[10];
            sprintf(dist, "%0.2f cm", distanciaS2 * 100);
            printf("Distancia DETRÁS: %0.0f cm\n", distanciaS2 * 100);
            
            //parar si hay obstáculo
            if((distanciaS2*100) < 4){
                stop();
            }
            if((distanciaS2*100) >= 4){ //continuar hacia atrás
                back();
            }


            if((strcmp(buf, "back") != 0 && strcmp(buf, "back_right") != 0 && strcmp(buf, "back_left") != 0) || strcmp(buf, "stop") == 0)
            // Libera el semáforo después de que se muestre la distancia del sensor
            xSemaphoreGive(xDistanceSemaphore);
            }
        else
        {
            printf("Atrás Error %d: %s\n", res, esp_err_to_name(res));
        }
